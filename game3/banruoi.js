...
let score = 0;
let lives = 3;
let invincible = false; // 👈 thêm dòng này
...
function moveBossBullets() {
    ...
    bossBullets.forEach(b => {
        b.y += 2 + level * 0.5; // tốc đọ đạn boss
    });
...
function collisionDetection() {
    ...
    for (let ci = chickens.length - 1; ci >= 0; ci--) {
        ...
        if (
            ...
            chickens.splice(ci, 1); // xóa con gà
...
function update() {
    ...
    // qua màn
    if (score >= scoreToNext && bossSpawned === false) {
...
function spawnChicken() {
    ...
    if (!gameOver) {
        ...
        let amount = 1 + Math.floor(level / 2);
